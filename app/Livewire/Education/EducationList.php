...
class EducationList extends Component
{
    ...
    public function createNewEducation()
    {
        ...
        $existingEntry = EducationalBackground::where('personal_information_id', $this->personalInformationId)
            ->where('level', $data['level'])
            ->first();

        if (in_array($this->level, ['elementary', 'secondary', 'college'])) {
            if ($existingEntry) {
                $level = ucwords($data['level']);
                $this->dispatch('alert', type: 'warning', message: "{$level} has already exist.", title: 'Education');
        ...
        $level = ucwords($education->level);
        $this->dispatch('alert', type: 'success', message: "{$level} has been saved.", title: 'Education');
        $this->dispatch('close-education-form-modal');
    }
